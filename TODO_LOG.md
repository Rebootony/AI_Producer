...
## Demo 追加改动记录
- ✅ 看板详情可查看：项目总览与执行排期卡片增加“查看详情”。
- ✅ 传话机制增强：支持 AI 主动向员工发起会话，并在员工回复后自动回传给老板。
- ✅ 角色简化：Demo 固定为 1 个老板 + 1 个员工，其它角色仅占位符。
- ✅ 模型可切换：支持通过环境变量接入本地免费模型或云端模型。
...
【第一步 快速给出报价和排期】
老板（商务）AI
1. 用户brief（粗略评估活儿的大小）+商务的交流（形式可能不定，比如日志记录）+自己的价格单（成本）
—》刊例价+利润率 =  报价
（天数）--〉预估排期（部分字段应可变）
2. 沟通
-> 出结果
...
-> 
【中期】
1. AI 应对批复员工提问（如预算是否能修改（做简单判断+审批）等）
2. 实时交互CRUD
